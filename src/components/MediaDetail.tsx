      ...
      try {
        if (media.mediaType === "tv") {
          const response = await fetch(
            `${baseApiUrl}/tv/${media.id}?language=en`,
            {
              headers: {
                'X-Api-Key': apiKey,
          ...
          const response = await fetch(
            `${baseApiUrl}/person/${media.id}?language=en`,
            {
              headers: {
                'X-Api-Key': apiKey,
                'accept': 'application/json',
          ...
          {media.mediaType === "person" ? (
            <>
              <Detail.Metadata.TagList title="Quick Info">
                ...
                {personDetails?.popularity && (
                  <Detail.Metadata.TagList.Item 
                    text={`Popularity: ${personDetails.popularity}`} 
                    color="yellow" 
                  />
                )}
                {personDetails?.adult && (
              ...
              {personDetails?.birthday && (
                <Detail.Metadata.Label 
                  title="Birthday" 
                  text={new Date(personDetails.birthday).toLocaleDateString()} 
                />
              )}
              ...
              {personDetails?.deathday && (
                <Detail.Metadata.Label 
                  title="Died" 
                  text={new Date(personDetails.deathday).toLocaleDateString()} 
                />
              )}
              ...
              {personDetails?.alsoKnownAs && personDetails.alsoKnownAs.length > 0 && (
                <Detail.Metadata.Label 
                  title="Also Known As" 
                  text={personDetails.alsoKnownAs.join(", ")} 
                />
              )}
            ...
            <>
              ...
              <Detail.Metadata.TagList title="Quick Info">
                ...
                {media.releaseDate && (
                  <Detail.Metadata.TagList.Item text={new Date(media.releaseDate).getFullYear().toString()} color="magenta" />
                )}
              ...
              {media.mediaType === "tv" && tvDetails && (
                <>
                  ...
                  {tvDetails.contentRatings?.results?.[0] && (
                    <Detail.Metadata.Label 
                      title="Rating" 
                      text={tvDetails.contentRatings.results[0].rating} 
                    />
                  )}
                  ...
                  {tvDetails.networks?.length > 0 && (
                    <Detail.Metadata.Label 
                      title="Networks" 
                      text={tvDetails.networks.map(n => n.name).join(", ")} 
                    />
                  )}
